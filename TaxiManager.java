...
import tamrintaxi.dao.DriverDao;
import tamrintaxi.dao.PassengerDao;
import tamrintaxi.dao.TripDao;
import tamrintaxi.dao.VehicleDao;
import tamrintaxi.enumaration.*;
...
public class TaxiManager {
    ...
    public static void handleExceptionForMobileNumFormat(String input) {
        ...
        if (!Pattern.matches(regex, input))
            throw new RuntimeException("some thing is not correct about this phone number");
    }

    public static void handleExceptionForPlaqueFormat(String input) {
        String regex = "[0-9]{2}[a-z][0-9]{2}";
        if (!Pattern.matches(regex, input))
            throw new RuntimeException("the format of plaque must be like: 99x99");
    }
    ...
    public void showAllDrivers() {
        List<Driver> drivers = driverDao.showAllDriver();
    ...
    public void signupOrLogin(int caseNum) throws InterruptedException {
        ...
        if (caseNum == 3) {
            Driver driver = driverDao.getDriverByPersonalCode(inputPersonalId);
            if (driver == null)
                registerOrExit("d");
            else {
    ...
    public void showAllOngoingTravels() {
        ...
        for (Trip trip : allTrips) {
            if (trip.getTripStatus().equals(TripStatus.ON_TRIP)) {
                ...
                Driver driver = driverDao.getDriverById(trip.getDriver().getId());
    ...
    public void showOptionsForDriverWhileTraveling(Driver driver) {
        ...
        do {
            ...
            switch (chosenOption) {
                case 1:
                    if (payment.equals(Payment.BY_ACCOUNT_BALANCE))
                    ...
                    continue;
                case 2:
                    if (payment.equals(Payment.BY_CASH) && !confirmCashReceipt) {
                        ...
                        tripDao.updateStatus(driverTrip, TripStatus.FINISH);
        ...
        while (true) {
            ...
            if (answer == 1 || answer == 2) {
                ...
                if (answer == 2) {
                    ...
                    payment = Payment.BY_ACCOUNT_BALANCE;
                } else
                    payment = Payment.BY_CASH;

                Trip trip = new Trip(passenger, origin, destination, cost, payment);
...
//            }
//        }
//        return driverDao.getDriverByLocation(dLocation);
//    }
}
